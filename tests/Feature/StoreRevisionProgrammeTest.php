...
class ReviseProgrammeTest extends TestCase
{
    ...
    /** @test */
    public function revision_of_programme_canbe_added()
    {
        ...
        foreach ($semester_courses as $index => $course) {
            $revision->attach($course->id, ['semester' => $index + 1]);
        }
       
        $revised_at = '2000-02-01';

        $this->post("/programmes/$programme->id/revision", ['revised_on' => $revised_at, 'semester_courses' => [[$semester_courses[0]->id], [$semester_courses[1]->id]]])
            ->assertRedirect('/programmes')
            ->assertSessionHasFlash('success', 'Programme revised successfully!');
       
        $this->assertEquals(1, Programme::count());
        $this->assertEquals($revised_at, Programme::find(1)->revision()->find(2));
    }
