@extends('layouts.scholar-profile', ['pageTitle' => 'Update Presentation', 'scholar' => $scholar])
@section('body')
    <div class="page-card max-w-xl mx-auto my-4">
        <div class="page-header flex items-baseline">
            <h2 class="mr-6">Update Presentation</h2>
        </div>
        <form action="{{ route('scholars.presentations.update', ['scholar' => $scholar, 'presentation' => $presentation])}}" method="post" class="px-6">
            @csrf_token
            @method('PATCH')
            <div class="mb-4">
