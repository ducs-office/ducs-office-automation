<div class="ml-auto mr-3 relative">
    ...
    <button class="relative z-20 flex items-center btn pl-2 pr-6 py-1" x-on:click="userDropdown = !userDropdown">
        <img src=" {{ auth()->guard('scholars')->user()->avatar_url }}"
            alt="{{ auth()->guard('scholars')->user()->name }}" width="32" height="32" class="w-6 h-6 rounded-full mr-2">
        <div class="leading-none text-left">
            <h2 class="font-bold truncate max-w-32">{{ auth()->guard('scholars')->user()->first_name }}</h2>
            <h3 class="text-xs text-black-50">Phd Scholar</h3>
        </div>
    </button>
