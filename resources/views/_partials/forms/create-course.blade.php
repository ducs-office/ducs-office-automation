<form action="{{ route('staff.courses.store') }}" method="POST"
    ...
    <div class="space-y-1">
        <label for="unique-course-code"
            class="w-full form-label @error('code', 'create') text-red-500 @enderror">
            Unique Course Code<span class="text-red-600">*</span>
        </label>
        <input id="unique-course-code" name="code" type="text"
            class="w-full form-input @error('code', 'create') border-red-500 hover:border-red-700 @enderror"
    ...
    <div class="space-y-1">
        <label for="course-name"
            class="w-full form-label @error('name', 'create') text-red-500 @enderror">
            Course Name <span class="text-red-600">*</span>
        </label>
        <input id="course-name" type="text" name="name"
            class="w-full form-input @error('name', 'create') border-red-500 hover:border-red-700 @enderror"
    ...
    <div class="space-y-1">
        <label for="course_type"
            class="w-full form-label @error('type', 'create') text-red-500 @enderror">
            Type <span class="text-red-600">*</span>
        </label>
        <select name="type"
            class="w-full form-select @error('type', 'create') border-red-500 hover:border-red-700 @enderror"
            x-model="type"
            required>
    ...
    <div class="flex items-center space-x-2 mb-2">
        <div class="w-1/2 space-y-1">
            <label for="course_revision_date"
                class="w-full form-label @error('date', 'create') text-red-500 @enderror">
                Syllabus Revision Date <span class="text-red-600">*</span>
            </label>
            <input type="date" name="date" id="course_revision_date"
            class="w-full form-input @error('date', 'create') border-red-500 hover:border-red-700 @enderror"
            value="{{ old('date') }}"
            required>
            @error('date', 'create')
        ...
        <div class="w-1/2 space-y-1">
            <label for="course_attachments"
                class="w-full form-label @error('attachments.*', 'create') text-red-500 @enderror">
