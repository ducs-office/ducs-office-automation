<h2 class="text-lg font-bold mb-8 flex items-center space-x-4">
    <img class="h-8 w-8 bg-gray-400 rounded-full overflow-hidden flex-shrink-0" src="{{ $scholar->avatar_url }}"
...
<form x-data="{cosupervisor_id: '{{ old('cosupervisor_id', optional($scholar->currentCosupervisor)->id) }}'}"
    ...
    <div class="space-y-1">
        ...
        <select id="replace-cosupervisor" class="form-select w-full block" name="cosupervisor_id"
            x-model="cosupervisor_id">
            <option selected value="">None</option>
            @foreach ($cosupervisors as $id => $name)
            <option value="{{ $id }}" @if(optional($scholar->currentSupervisor)->id == $id) disabled @endif>
                {{ $name }}
            </option>
            @endforeach
        </select>
