<form action="{{ route('scholars.progress-reports.store', $scholar) }}" method="POST"
    ...
    <div class="space-y-1">
        <label for="date" class="mb-1 w-full form-label"
            @error('date', 'progressReport') text-red-500 @enderror">
            Date <span class="text-red-600">*</span>
        </label>
        <input type="date" name="date" id="date"
        class="w-full form-input @error('date', 'progressReport') border-red-500 hover:border-red-700 @enderror">
        @error('date', 'progressReport')
            <p class="text-red-500">{{ $message }}</p>
        @enderror
    </div>
    <div class="space-y-1">
        <label for="recommendation"
            class="mb-1 w-full form-label @error('recommendation', 'progressReport') text-red-500 @enderror">
            Recommendation <span class="text-red-600">*</span>
        </label>
        <select name="recommendation" x-model="recommendation"
            class="w-full form-select @error('recommendation', 'progressReport') border-red-500 hover:border-red-700 @enderror">
        ...
        @error('recommendation', 'progressReport')
            <p class="text-red-500">{{ $message }}</p>
        @enderror
    </div>
    <div class="space-y-1">
        <label for="progress_report"
            class="w-full form-label mb-1 @error('progress_report', 'progressReport') text-red-500 @enderror">
        ...
        <x-input.file name="progress_report" id="progress_report" class="w-full form-input inline-flex items-center {{ $errors->has('progress-report') ? 'border-red-500 hover:border-red-700' : '' }}"
            ...
            required/>
        @error('progress_report', 'progressReport')
            <p class="text-red-500">{{ $message }}</p>
        @enderror
    </div>
